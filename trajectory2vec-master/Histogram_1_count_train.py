...
def vecClusterAnalysis(hists):
    ...
    for i in range(REPEAT_TIME):
        ...
        clf = SVC(kernel='linear')      #使用SVM算法进行分类训练，下面是可以替代采用的分类算法
...
if __name__ == '__main__':
    for i in range(20):
        hists=read_data_hist('highbeta', 2, i+1)   #将最大通道和能量信息读取成轨迹信息，前两个参数为指定的频率段和condition
        vecClusterAnalysis(hists)
    sio.savemat('../counts.mat' , {'counts':counts})	#保存直方图数据，以供后续处理