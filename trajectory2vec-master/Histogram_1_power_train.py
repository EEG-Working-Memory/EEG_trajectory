#使用直方图统计能量轨迹中各区域连线出现时的能量值，选出有效的连线作为特征进行分类训练
import random
import cPickle
import h5py
import numpy as np
...
def read_data_hist(band='alpha',con=1,person=1):
    ...
    for i in eventsConx:
        ...
        for j in range(23):
            if(allevents[i][con-1]==13):
                hist1[int(currentTrial[j][1]-1)][int(currentTrial[j+1][1]-1)]+=(currentTrial[j][0]+currentTrial[j+1][0])/2
            else:
                hist1[int(currentTrial[j][1]-1)][int(currentTrial[j+1][1]-1)]-=(currentTrial[j][0]+currentTrial[j+1][0])/2
    #sio.savemat('../%s_con%d_person%d_hist.mat' %(band, con, person), {'hist':hist1})
    #挑选有效的连线                
    goodIndex = []
    ...
    for i in eventsConx:
        ...
        currentTrac = []
        for j in range(23):
            hist[int(currentTrial[j][1]-1)][int(currentTrial[j+1][1]-1)]+=(currentTrial[j][0]+currentTrial[j+1][0])/2
        hists.append(hist)
    newHists = []
    for hist in hists:
        newHist=[]
