#对读取出的轨迹数据利用"Trajectory clustering via deep representation learning"配合SVM进行分类
import random
import cPickle
import numpy as np
import math
    ...
    for simTrjComps in simTrjCompss:
        ...
        for i in range(0,len(simTrjComps)):
            ...
            if(i==0):
                ...
                rec.append(simTrjComps[i][1])
                rec.append(math.sqrt((simTrjComps[i][2]-simTrjComps[i-1][2])**2+(simTrjComps[i][3]-simTrjComps[i-1][3])**2))
                if(simTrjComps[i][2]-simTrjComps[i-1][2]==0):
                    if(simTrjComps[i][3]-simTrjComps[i-1][3]>0):
                        rec.append(math.pi/2)
                    else:
                        rec.append(-math.pi/2)
                else:
                    rec.append(math.atan((simTrjComps[i][3]-simTrjComps[i-1][3])/ (simTrjComps[i][2]-simTrjComps[i-1][2])))
                    '''
            '''
            if i==0:
                # time, locationC, speedC, rotC
...
def trajectory2Vec(band='alpha',con=1,person=1):
    def loopf(prev, i):
        return prev
    ...
    with tf.Session() as sess:
        ...
        for input_data in input_datas:
            ...
            j = j + 1
        fout = file('./band_data/traj_vec_normal_reverse_{}_con{}_person{}'.format(band,con,person), 'w')
        cPickle.dump(trajectoryVecs, fout)

def vecClusterAnalysis(band='alpha',con=1,person=1):
    #input_datas = np.array(cPickle.load(open('./band_data/traj_vec_normal_reverse_{}_con{}_person{}'.format(band,con,person))))
    input_datas = []
    trs = cPickle.load(open('./band_data/traj_vec_normal_reverse_{}_con{}_person{}'.format(band,con,person)))
    for tr in trs:
    ...
    for i in range(REPEAT_TIME):
        ...
        total+=acc
    print("Test Accuracy of SVM = {:.2f} %".format(total * 100 / REPEAT_TIME))
    accs.append(total / REPEAT_TIME)
...
def print_accs(band='alpha',con=1):
    accs_data = cPickle.load(open('./band_data/deep_svm_con{}_{}'.format(con,band)))
    print '---------------------------------'
    for i in accs_data:
        print i
...
if __name__ == '__main__':
    ...
    print_accs('theta',1)
    print_accs('alpha',1)
    print_accs('lowbeta',1)
    print_accs('highbeta',1)
    print_accs('lowgamma',1)
    print_accs('highgamma',1)