#将能量轨迹分为三段，然后对每一段使用直方图统计能量轨迹中各区域连线出现次数，选出有效的连线作为特征进行分类训练
import random
import cPickle
import h5py
import numpy as np
...
def read_data_hist(band='alpha',con=1,person=1):
    ...
    for i in eventsConx:
        ...
        for j in range(16,23):
            if(allevents[i][con-1]==13):
                hist3[int(currentTrial[j][1]-1)][int(currentTrial[j+1][1]-1)]+=1
            else:
                hist3[int(currentTrial[j][1]-1)][int(currentTrial[j+1][1]-1)]-=1
    ...
    for indexes in range(26):
        for index in range(26):
            ...
            if hist2[indexes][index] >= Threshold or hist1[indexes][index] <= -Threshold:
                goodIndex.append([indexes,index])
            if hist3[indexes][index] >= Threshold or hist1[indexes][index] <= -Threshold:
                goodIndex.append([indexes,index])
    hists = []
    for i in eventsConx:
        hist = np.zeros((26, 26))
